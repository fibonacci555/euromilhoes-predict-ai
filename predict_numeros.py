...
for i in range(1337):
    edicao = [ws[f'B{str(2+i)}'].value,ws[f'C{str(2+i)}'].value,ws[f'D{str(2+i)}'].value,ws[f'E{str(2+i)}'].value,ws[f'F{str(2+i)}'].value]
...
print(scaler.inverse_transform(scaled_to_predicted_output_1).astype(int)[0])